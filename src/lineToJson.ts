...
export default async function (
  ...
  for (let i = 0, len = csvRows.length; i < len; i++) {
    const r = await processRow(csvRows[i], conv, i);
    if (r) {
      res.push(r);
    }
  }
...
async function processRow(
  ...
  const resultRow = await convertRowToJson(row, headRow, conv);
  if (resultRow) {
    return resultRow;
  } else {
    return null;
...
async function convertRowToJson(
  row: string[],
  headRow: string[],
  conv: Converter
  ...
  for (let i = 0, len = row.length; i < len; i++) {
    ...
    if (convFunc) {
      const convRes = await convFunc(item, head, resultRow, row, i);
